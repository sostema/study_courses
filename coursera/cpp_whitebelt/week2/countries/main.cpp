    ...
    if (operation == "CHANGE_CAPITAL") {
      ...
      if (countryCapital.count(country)) {
        if (countryCapital[country] == capital) {
          ...
          std::cout << "Country " << country << " has changed its capital from " << countryCapital[country] << " to "
                    << capital << std::endl;
      ...
      if (countryCapital.count(old_country_name) && old_country_name != new_country_name
          && countryCapital.count(new_country_name) == 0) {
        std::cout << "Country " << old_country_name << " with capital " << countryCapital[old_country_name]
